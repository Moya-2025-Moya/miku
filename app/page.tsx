...
// Seed library so the list looks alive before Supabase has data / when it's not
// configured. Real profiles from the API are merged on top by name.
...
// so text fields can "type out" as the analysis streams in.
...
export default function Home() {
  // mview drives the mobile single-column view stack; desktop shows all 3 panes.
  ...
  // Desktop opens onto a conversation (3-pane); mobile still starts on the list.
  useEffect(() => {
    setActive((cur) => cur ?? profiles[0] ?? null);
  }, [profiles]);
  ...
  function openConversation(p: Profile) {
    setActive(p);
    setSelected(new Set());
    setAnalysis(null);
    setError(null);
  ...
  async function analyze() {
    ...
    setPanelOpen(true);
    try {
      // Only archive/learn against a real (Supabase-backed) profile; seeded demo
      // cards fall back to stateless analysis.
      ...
      for (;;) {
        ...
        for (const event of events) {
          ...
          if (payload.text) {
            if (firstText) { setLoading(false); firstText = false; }
            json += payload.text;
    ...
    <div className="sr-app" data-mview={mview} data-panel={panelOpen ? "open" : "closed"}>
      ...
      <aside className="sr-sidebar">
        <div style={{ padding: "20px 18px 14px", borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <div style={{ fontFamily: "Fraunces, serif", fontSize: 22, fontWeight: 600, color: C.teal, letterSpacing: "-0.02em" }}>
            ScreenRead
          </div>
          <div style={{ fontSize: 11.5, color: C.muted, marginTop: 2 }}>by Miku · powered by Claude</div>
        </div>

        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", padding: "14px 18px 8px" }}>
          <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>
            Conversations
          </span>
          <span style={{ fontSize: 11.5, color: C.muted }}>{profiles.length}</span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 10px 10px", display: "flex", flexDirection: "column", gap: 2 }}>
          {profiles.map((p) => {
            ...
            return (
              <button key={p.id} className={`sr-convo${isActive ? " is-active" : ""}`} onClick={() => openConversation(p)}>
                <div
                  style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    <span style={{ fontWeight: 600, fontSize: 14.5, color: C.text }}>{p.name}</span>
                    {p.relationship_type && (
                      <span style={{ fontSize: 10, fontWeight: 600, color: C.teal, background: `${C.teal}14`, padding: "1px 7px", borderRadius: 99, textTransform: "capitalize" }}>
                        {p.relationship_type}
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {p.tagline ?? "Open the conversation"}
                  </div>
                </div>
              </button>
            );
      ...
      {/* ── Right: Miku analysis (desktop column / mobile bottom sheet) ───── */}
      <aside className="sr-aside">
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "18px 20px 14px", borderBottom: "1px solid rgba(15,118,110,0.1)", position: "sticky", top: 0, background: C.cream, zIndex: 5 }}>
          ...
          <button
            className="sr-sheet-close"
            onClick={() => setPanelOpen(false)}
            style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(0,0,0,0.07)", border: "none", cursor: "pointer", fontSize: 18, color: C.muted, alignItems: "center", justifyContent: "center" }}
          >
            ×
          </button>
        </div>
        ...
        {error ? (
          <div style={{ padding: "14px 14px 28px", display: "flex", flexDirection: "column", gap: 10 }}>
            <div className="fade-up" style={{ background: "#fff", borderRadius: 18, padding: "18px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: `${C.coral}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>⚠</div>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: C.coral }}>Error</span>
              </div>
              <p style={{ fontFamily: "Fraunces, serif", fontSize: 15, lineHeight: 1.6, color: C.text, margin: "0 0 16px" }}>{error}</p>
              <button onClick={() => setError(null)} style={{ padding: "9px 20px", borderRadius: 99, background: C.teal, color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Try again</button>
            </div>
          </div>
        ) : analysis ? (
          <div style={{ padding: "14px 14px 28px", display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {/* Vibe read */}
            <div className="fade-up" style={{ background: "#fff", borderRadius: 18, padding: "14px 16px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", animationDelay: "0.04s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 9 }}>
                <span style={{ fontSize: 15 }}>👁</span>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: C.teal }}>Vibe read</span>
              </div>
              <p style={{ fontFamily: "Fraunces, serif", fontSize: 14.5, lineHeight: 1.65, color: C.text, margin: 0 }}>{analysis.vibe_read}</p>
            </div>

            {/* Reality check */}
            <div className="fade-up" style={{ background: "#fff", borderRadius: 18, padding: "14px 16px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", animationDelay: "0.09s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 9 }}>
                <span style={{ fontSize: 15 }}>🔍</span>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: C.coral }}>Reality check</span>
              </div>
              <p style={{ fontSize: 13.5, lineHeight: 1.65, color: C.sub, margin: 0 }}>{analysis.reality_check}</p>
            </div>

            {/* Reply drafts */}
            {analysis.response_options?.length > 0 && (
              <div className="fade-up" style={{ background: "#fff", borderRadius: 18, padding: "14px 16px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", animationDelay: "0.14s" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                  <span style={{ fontSize: 15 }}>💬</span>
                  <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: C.muted }}>Reply drafts</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {analysis.response_options.map((opt, i) => {
                    ...
                    return (
                      <div key={i} className="fade-up" onClick={() => copyDraft(i, opt.draft)} style={{ borderLeft: `3px solid ${tc}`, borderRadius: "0 12px 12px 0", padding: "10px 12px", background: `${tc}09`, cursor: "pointer", animationDelay: `${0.16 + i * 0.05}s` }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 }}>
                          <span style={{ fontSize: 10, fontWeight: 600, color: tc, textTransform: "capitalize", letterSpacing: "0.04em" }}>{opt.tone}</span>
                          <span style={{ fontSize: 10, color: done ? "#16a34a" : C.muted, fontWeight: done ? 600 : 400, transition: "color 0.2s" }}>{done ? "✓ copied" : "copy"}</span>
                        </div>
                        <div style={{ fontSize: 13, lineHeight: 1.5, color: C.text }}>{opt.draft}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Verdict */}
            <div className="fade-up" onClick={copyVerdict} style={{ borderRadius: 20, padding: "18px", background: `linear-gradient(140deg, ${C.teal} 0%, ${C.tealDark} 100%)`, cursor: "pointer", animationDelay: "0.19s", boxShadow: `0 10px 28px ${C.teal}45` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
                <span style={{ fontSize: 14 }}>⚡</span>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.5)" }}>Verdict</span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 11, color: copiedVerdict ? "#86efac" : "rgba(255,255,255,0.45)", fontWeight: copiedVerdict ? 600 : 400, transition: "color 0.2s" }}>{copiedVerdict ? "✓ copied" : "tap to copy"}</span>
              </div>
            </div>

            {/* Follow-up chat */}
            <div className="fade-up" style={{ background: "#fff", borderRadius: 18, padding: "14px 16px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", animationDelay: "0.24s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                <span style={{ fontSize: 15 }}>✦</span>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: C.muted }}>Ask a follow-up</span>
              </div>
              {followUps.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 10 }}>
                  {followUps.map((m, i) => (
                    <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "90%", background: m.role === "user" ? C.teal : C.cream, color: m.role === "user" ? "#fff" : C.text, borderRadius: m.role === "user" ? "14px 14px 3px 14px" : "14px 14px 14px 3px", padding: "8px 12px", fontSize: 13, lineHeight: 1.5 }}>
                      {m.text}
                    </div>
                  ))}
                  {chatLoading && (
                    <div style={{ alignSelf: "flex-start", color: C.muted, padding: "4px 10px", fontSize: 20, letterSpacing: 2 }}>···</div>
                  )}
                  <div ref={chatEndRef} />
                </div>
              )}
              <div style={{ display: "flex", gap: 8, background: "#f7f7f7", borderRadius: 14, padding: "4px 4px 4px 14px", border: `1px solid ${C.teal}1a` }}>
                <input
                  value={chatInput}
                  ...
                  style={{ flex: 1, border: "none", outline: "none", fontSize: 13, background: "transparent", color: C.text, fontFamily: "Inter, sans-serif", padding: "8px 0" }}
                />
                <button onClick={sendFollowUp} disabled={chatLoading || !chatInput.trim()} style={{ width: 34, height: 34, borderRadius: 10, background: chatInput.trim() ? C.teal : "#e5e7eb", color: "#fff", border: "none", cursor: chatInput.trim() ? "pointer" : "default", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.2s" }}>↑</button>
              </div>
            </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "52px 32px", textAlign: "center", gap: 16 }}>
            ...
            <div style={{ marginTop: 4, fontSize: 11, color: C.muted, opacity: 0.6, padding: "5px 12px", background: "rgba(0,0,0,0.04)", borderRadius: 99 }}>
